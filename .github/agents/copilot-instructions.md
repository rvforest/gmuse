...
- dev
  ...
  - Ruff (lint/format)
  - pyrefly
- Python 3.10+ + Typer (CLI), LiteLLM (model/provider parsing and request execution), tomllib/tomli (config loading), pytest (tests), Ruff (lint/format), pyrefly (type checking) (main)
- Filesystem config (`~/.config/gmuse/config.toml` by default), environment variables, in-memory resolution context, and git repository state for generation inputs (main)
...
## Recent Changes
- main: Added Python 3.10+ + Typer (CLI), LiteLLM (model/provider parsing and request execution), tomllib/tomli (config loading), pytest (tests), Ruff (lint/format), pyrefly (type checking)

...
def test_gmuse_conventional_types_and_scopes_are_accepted() -> None:
    result = validate_assets(assets(allowed_types=["fix", "test"]), "custom", reconstruct=False)

    assert result.report.status == "passed"


def test_unsupported_conventional_type_is_reported() -> None:
    result = validate_assets(assets(allowed_types=["build"]), "custom", reconstruct=False)

    assert "unsupported_conventional_type" in {issue.code for issue in result.report.errors}
